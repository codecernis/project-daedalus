...
var planets = [
  {
    ...
    desc: 'Spearheaded the DuitNow QR cashless payment rollout nationwide for PayNet, including post-COVID adoption analysis across 100+ merchants. Delivered high-frequency data analytics and strategic reporting for blue-chip clients including Celcom and Maybank. Also built a virtual showroom web app for Sime Darby Properties — my first taste of building something people actually used.'
  },
...

  {
    ...
    desc: 'Built Python automation frameworks for treasury transactions worth more than I\'ll ever see in my bank account — we\'re talking RM 100 million+ deals. Cut manual processing time by 50% and reduced operational risk in the process. Also engineered custom Murex tracking modules for Global Markets data reconciliation. The role where I learned that in banking, \'move fast and break things\' is not a philosophy anyone appreciates.'
  },
]
...
function getExpPlanetPos(p, t) {
  var a = Math.min(SW, SH) * p.orbitA;
  var b = Math.min(SW, SH) * p.orbitB;
  var angle = p.angle + t * p.speed;
  return {
    x: sunCenterX + Math.cos(angle) * b,
    y: sunCenterY + Math.sin(angle) * a
  };
}

function initExpShip() {
  var pos = getExpPlanetPos(expPlanets[0], solarTime);
  expShipX = pos.x;
  expShipY = pos.y;
  expTargetX = pos.x;
  expTargetY = pos.y;
...
function drawSolarSystem() {
  ...
  for (var i = 0; i < bgStars.length; i++) {
    var s = bgStars[i];
    var twinkle = Math.sin(solarTime * s.twinkleSpeed + s.twinkleOffset) * 0.25 + 0.3;
    solarCtx.beginPath();
    solarCtx.arc(s.x * SW, s.y * SH, s.size, 0, Math.PI * 2);
    solarCtx.fillStyle = 'rgba(205, 214, 244, ' + twinkle + ')';
    solarCtx.fill();
  }
  ...
  var sunGlow3 = solarCtx.createRadialGradient(sunCenterX, sunCenterY, sunRadius, sunCenterX, sunCenterY, sunRadius + 50 + sunPulse);
  sunGlow3.addColorStop(0, 'rgba(249, 226, 175, 0.06)');
  sunGlow3.addColorStop(1, 'rgba(249, 226, 175, 0)');
  solarCtx.beginPath();
  solarCtx.arc(sunCenterX, sunCenterY, sunRadius + 50 + sunPulse, 0, Math.PI * 2);
  solarCtx.fillStyle = sunGlow3;
  solarCtx.fill();

  var sunGlow2 = solarCtx.createRadialGradient(sunCenterX, sunCenterY, sunRadius * 0.5, sunCenterX, sunCenterY, sunRadius + 25 + sunPulse);
  sunGlow2.addColorStop(0, 'rgba(250, 179, 135, 0.12)');
  sunGlow2.addColorStop(1, 'rgba(250, 179, 135, 0)');
  solarCtx.beginPath();
  solarCtx.arc(sunCenterX, sunCenterY, sunRadius + 25 + sunPulse, 0, Math.PI * 2);
  solarCtx.fillStyle = sunGlow2;
  solarCtx.fill();

  var sunGrad = solarCtx.createRadialGradient(
    sunCenterX - sunRadius * 0.3, sunCenterY - sunRadius * 0.3, sunRadius * 0.1,
    sunCenterX, sunCenterY, sunRadius
  );
  sunGrad.addColorStop(0, '#f9e2af');
  sunGrad.addColorStop(0.4, '#fab387');
  sunGrad.addColorStop(1, '#e8834a');
  solarCtx.beginPath();
  solarCtx.arc(sunCenterX, sunCenterY, sunRadius, 0, Math.PI * 2);
  solarCtx.fillStyle = sunGrad;
  solarCtx.fill();

  solarCtx.beginPath();
  solarCtx.arc(sunCenterX - sunRadius * 0.2, sunCenterY - sunRadius * 0.2, sunRadius * 0.3, 0, Math.PI * 2);
  solarCtx.fillStyle = 'rgba(255, 255, 255, 0.12)';
  solarCtx.fill();

  // Sun label
  solarCtx.font = '12px JetBrains Mono';
  solarCtx.fillStyle = 'rgba(249, 226, 175, 0.8)';
  solarCtx.textAlign = 'center';
  solarCtx.fillText('Data', sunCenterX, sunCenterY + sunRadius + 18);
  solarCtx.textAlign = 'start';
  ...
  for (var i = 0; i < expPlanets.length; i++) {
    ...
    solarCtx.beginPath();
    solarCtx.ellipse(sunCenterX, sunCenterY, b, a, 0, 0, Math.PI * 2);
    if (i === activeExp) {
      solarCtx.strokeStyle = 'rgba(' + p.color + ', 0.18)';
      solarCtx.lineWidth = 1;
    } else {
      solarCtx.strokeStyle = 'rgba(88, 91, 112, 0.15)';
  ...
  for (var i = 0; i < expPlanets.length; i++) {
    var pos = getExpPlanetPos(expPlanets[i], solarTime);
    planetPositions.push({ index: i, x: pos.x, y: pos.y });
  }
  ...
  for (var pi = 0; pi < planetPositions.length; pi++) {
    ...
    // Moon
    if (p.hasMoon) {
      var moonAngle = solarTime * p.moonSpeed;
      var moonX = px + Math.cos(moonAngle) * p.moonOrbit;
      var moonY = py + Math.sin(moonAngle) * p.moonOrbit * 0.35;

      solarCtx.beginPath();
      solarCtx.ellipse(px, py, p.moonOrbit, p.moonOrbit * 0.35, 0, 0, Math.PI * 2);
      solarCtx.strokeStyle = 'rgba(' + p.color + ', 0.12)';
      solarCtx.lineWidth = 0.5;
      solarCtx.stroke();

      solarCtx.beginPath();
      solarCtx.arc(moonX, moonY, 2.5, 0, Math.PI * 2);
      solarCtx.fillStyle = 'rgba(' + p.color + ', 0.4)';
      solarCtx.fill();
    }

    // Planet glow
    var glowPulse = Math.sin(solarTime * 0.018 + idx * 1.5) * 2;
    var glowSize = p.size + 12 + glowPulse;
    var glow = solarCtx.createRadialGradient(px, py, p.size * 0.3, px, py, glowSize);
    glow.addColorStop(0, 'rgba(' + p.color + ', 0.15)');
    glow.addColorStop(1, 'rgba(' + p.color + ', 0)');
    solarCtx.beginPath();
    solarCtx.arc(px, py, glowSize, 0, Math.PI * 2);
    ...
    var pGrad = solarCtx.createRadialGradient(
      px - p.size * 0.3, py - p.size * 0.3, p.size * 0.1,
      px + p.size * 0.1, py + p.size * 0.1, p.size
    );
    pGrad.addColorStop(0, 'rgba(' + p.color + ', 1)');
    pGrad.addColorStop(0.6, 'rgba(' + p.color + ', 0.75)');
    pGrad.addColorStop(1, 'rgba(' + p.color + ', 0.3)');
    solarCtx.fillStyle = pGrad;
    solarCtx.fill();

    // Specular highlight
    solarCtx.beginPath();
    solarCtx.arc(px - p.size * 0.25, py - p.size * 0.25, p.size * 0.22, 0, Math.PI * 2);
    solarCtx.fillStyle = 'rgba(255, 255, 255, 0.1)';
    solarCtx.fill();

    // Active ring
    if (idx === activeExp) {
      solarCtx.beginPath();
      solarCtx.arc(px, py, p.size + 5, 0, Math.PI * 2);
      solarCtx.strokeStyle = 'rgba(' + p.color + ', 0.5)';
      solarCtx.lineWidth = 1.5;
      solarCtx.setLineDash([3, 3]);
      solarCtx.stroke();
      solarCtx.setLineDash([]);
    }

    // Label
    solarCtx.font = '10px JetBrains Mono';
    solarCtx.fillStyle = 'rgba(' + p.color + ', 0.65)';
    solarCtx.textAlign = 'center';
    solarCtx.fillText(p.name, px, py + p.size + 16);
    solarCtx.textAlign = 'start';
  }

  // Spaceship
  var activePos = getExpPlanetPos(expPlanets[activeExp], solarTime);
  expTargetX = activePos.x;
  expTargetY = activePos.y;

  var dx = expTargetX - expShipX;
  var dy = expTargetY - expShipY;
  var dist = Math.sqrt(dx * dx + dy * dy);

  if (dist > expPlanets[activeExp].size + 8) {
    var speed = Math.min(dist * 0.03, 4.5);
    expShipX += (dx / dist) * speed;
    expShipY += (dy / dist) * speed;
    expShipAngle = Math.atan2(dy, dx);
    expShipTrail.push({ x: expShipX, y: expShipY, age: 0 });
    if (expShipTrail.length > 20) expShipTrail.shift();
  } else {
    var orbitDist = expPlanets[activeExp].size + 10;
    var orbitAngle = solarTime * 0.02;
    expShipX = activePos.x + Math.cos(orbitAngle) * orbitDist;
    expShipY = activePos.y + Math.sin(orbitAngle) * orbitDist * 0.6;
    expShipAngle = orbitAngle + Math.PI * 0.5;
  }

  // Ship trail
  for (var i = 0; i < expShipTrail.length; i++) {
    expShipTrail[i].age += 1;
    var alpha = (1 - expShipTrail[i].age / 45) * 0.3;
    var trailSize = 1.2 * (1 - i / expShipTrail.length);
    if (alpha > 0) {
      solarCtx.beginPath();
      solarCtx.arc(expShipTrail[i].x, expShipTrail[i].y, trailSize, 0, Math.PI * 2);
      solarCtx.fillStyle = 'rgba(250, 179, 135, ' + alpha + ')';
      solarCtx.fill();
    }
  }
  ...
  // Ship body
  solarCtx.save();
  solarCtx.translate(expShipX, expShipY);
  solarCtx.rotate(expShipAngle);

  solarCtx.beginPath();
  solarCtx.moveTo(8, 0);
  solarCtx.lineTo(-4, -3.5);
  solarCtx.lineTo(-1.5, 0);
  solarCtx.lineTo(-4, 3.5);
  solarCtx.closePath();
  solarCtx.fillStyle = '#cdd6f4';
  solarCtx.fill();

  solarCtx.beginPath();
  solarCtx.moveTo(-4, -2.5);
  solarCtx.lineTo(-6.5, 0);
  solarCtx.lineTo(-4, 2.5);
  solarCtx.closePath();
  solarCtx.fillStyle = 'rgba(137, 180, 250, 0.4)';
  solarCtx.fill();

  if (dist > expPlanets[activeExp].size + 10) {
    solarCtx.beginPath();
    solarCtx.moveTo(-1.5, 0);
    solarCtx.lineTo(-7 - Math.random() * 4, -1.5 + Math.random() * 3);
    solarCtx.lineTo(-5 - Math.random() * 2, 0);
    solarCtx.lineTo(-7 - Math.random() * 4, 1.5 - Math.random() * 3);
    solarCtx.closePath();
    solarCtx.fillStyle = 'rgba(250, 179, 135, ' + (0.35 + Math.random() * 0.3) + ')';
    solarCtx.fill();
  }
  solarCtx.restore();
